...
async function queueVerificationEmail(userId, email, locale = "en") {
  db.prepare("UPDATE email_verification_tokens SET used = 1 WHERE user_id = ? AND used = 0").run(
    userId
  );
  const token = uid() + uid();
  ...
  const link = buildAccountAuthUrl("/account/verify-email", {
    locale: normalizeLocale(locale),
    token,
  });
  logAuthEmailLink("verify-email", link);
...
router.post("/register", async (req, res) => {
  ...
  try {
    emailResult = await queueVerificationEmail(id, email, locale);
  } catch (err) {
...
router.post("/forgot-password", forgotLimiter, async (req, res) => {
  ...
  if (row) {
    ...
    const resetUrl = buildAccountAuthUrl("/account/reset-password", { locale, token });
    logAuthEmailLink("reset-password", resetUrl);
    ...
    console.log("[auth] forgot-password processed", { email, mailSent: mail.sent });
  ...
  console.log("[auth] forgot-password requested (no matching user)", { email });
...
router.post("/reset-password", (req, res) => {
  ...
  if (!row || new Date(row.expires_at) < new Date()) {
    console.warn("[auth] reset-password failed", {
      reason: !row ? "invalid-token" : "expired-token",
    });
  ...
  console.log("[auth] reset-password success", { userId: row.user_id });
...
router.post("/send-verification", requireAuth, verifyLimiter, async (req, res) => {
  ...
  const locale = localeFromRequest(req);
  const result = await queueVerificationEmail(row.id, row.email, locale);
  res.json({
    ok: true,
    message: result.sent
...
/** Resend verification when user is not logged in (expired email link). */
router.post("/resend-verification-email", forgotLimiter, async (req, res) => {
  const email = cleanEmail(req.body.email);
  const locale = localeFromRequest(req);
  if (!email) return res.status(400).json({ error: "Email is required." });
  ...
  if (row && !row.email_verified) {
    await queueVerificationEmail(row.id, row.email, locale);
    console.log("[auth] resend-verification-email sent", { email });
  } else {
    console.log("[auth] resend-verification-email noop", { email, found: !!row });
  }

  res.json({
    success: true,
    message: "If your email is registered and not yet verified, a new verification link was sent.",
  });
});
...
router.get("/verify-email", (req, res) => {
  ...
  console.log("[auth] verify-email success", { userId: row.user_id });
